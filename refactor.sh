...
BASE_DIR="/Users/pierre/y/Experiments/LLMCLI/AudioTranscription/SegmentsReader"

mkdir -p "$BASE_DIR/Models" "$BASE_DIR/Views" "$BASE_DIR/Components" "$BASE_DIR/Extensions"

cat << 'SWIFT' > "$BASE_DIR/SegmentsReaderApp.swift"
#if os(macOS)
import AppKit
#endif
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Models/SegmentsReaderModel.swift"
#if os(macOS)
import AppKit
#endif
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Models/PopupModel.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Models/PhoneticCell.swift"
import Foundation
...
cat << 'SWIFT' > "$BASE_DIR/Models/SegmentLine.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Views/SegmentsReaderView.swift"
import SwiftUI
import UniformTypeIdentifiers
...
cat << 'SWIFT' > "$BASE_DIR/Views/SegmentScrollView.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Views/SegmentGroupView.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Components/IPAFontSizeControl.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Components/PhoneticScoreView.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Components/PhoneticCellView.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Components/CellPopupView.swift"
import SwiftUI
...
cat << 'SWIFT' > "$BASE_DIR/Extensions/String+Trim.swift"
import Foundation
...
cat << 'SWIFT' > "$BASE_DIR/Extensions/SegmentRecord+Phonetics.swift"
import Foundation
...
cat << 'SWIFT' > "$BASE_DIR/Extensions/ChineseCharacterUnit+IPA.swift"
import Foundation
...
cat << 'RUBY' > /tmp/update_pbxproj.rb
require 'xcodeproj'
project_path = '/Users/pierre/y/Experiments/LLMCLI/AudioTranscription/AudioTranscription.xcodeproj'
project = Xcodeproj::Project.open(project_path)
target = project.targets.find { |t| t.name == 'SegmentsReader' }

group = project.main_group.find_subpath('SegmentsReader', true)

models_group = group.find_subpath('Models', true)
views_group = group.find_subpath('Views', true)
components_group = group.find_subpath('Components', true)
extensions_group = group.find_subpath('Extensions', true)

files_to_add = {
  'Models/SegmentsReaderModel.swift' => models_group,
  'Models/PopupModel.swift' => models_group,
  'Models/PhoneticCell.swift' => models_group,
  'Models/SegmentLine.swift' => models_group,
  'Views/SegmentsReaderView.swift' => views_group,
  'Views/SegmentScrollView.swift' => views_group,
  'Views/SegmentGroupView.swift' => views_group,
  'Components/IPAFontSizeControl.swift' => components_group,
  'Components/PhoneticScoreView.swift' => components_group,
  'Components/PhoneticCellView.swift' => components_group,
  'Components/CellPopupView.swift' => components_group,
  'Extensions/String+Trim.swift' => extensions_group,
  'Extensions/SegmentRecord+Phonetics.swift' => extensions_group,
  'Extensions/ChineseCharacterUnit+IPA.swift' => extensions_group
}

files_to_add.each do |path, parent_group|
  full_path = "/Users/pierre/y/Experiments/LLMCLI/AudioTranscription/SegmentsReader/#{path}"
  file_ref = parent_group.files.find { |f| f.path == path.split('/').last }
  if file_ref.nil?
